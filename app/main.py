...
from app.core.retrieval import retrieve, should_answer
...
def _sidebar_settings(paths: "object") -> None:
    with st.sidebar:
...
def main() -> None:
    ...
    if page == "Upload":
        ...
        document_id = str(doc_labels[chosen_label])
        st.session_state.selected_document_id = document_id

        chat = _get_chat(document_id)
        top_bar = st.columns([1, 1, 1.2])
        with top_bar[0]:
            if st.button("Reset chat"):
                _reset_chat(document_id)
                st.rerun()
        with top_bar[1]:
            pass
        with top_bar[2]:
                    ...
                    with st.expander("Sources"):
                        for s in sources:
                            ...
                            if st.session_state.show_debug_scores and score is not None:
                                st.markdown(f"**Page {page_n}**  ·  score `{float(score):.3f}`")
                            else:
                                st.markdown(f"**Page {page_n}**")
        ...
        if prompt and normalize_query(prompt):
            ...
            with st.chat_message("assistant"):
                with st.spinner("Searching your notes…"):
                    ...
                    st.write(ans.text)
                    sources_payload = [
                        {"page_number": s.page_number, "excerpt": s.excerpt, "score": s.score} for s in ans.sources
                    ]
                    with st.expander("Sources", expanded=False):
                        for s in ans.sources:
                            if st.session_state.show_debug_scores:
                                ...
                                st.markdown(f"**Page {s.page_number}**")
                            st.write(s.excerpt)
                            st.divider()
